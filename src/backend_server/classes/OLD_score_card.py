...
class ScoreCard:
    def __init__(self, url: str):
        ...
        self.reviewedness = Reviewedness()
    ...
    def getName(self, url: str) -> str:
        ...
        if len(parts) < 2:
            raise ValueError(
                "Invalid HF URL; expected https://huggingface.co/<owner>/<repo>"
            )
        return parts[1]
    ...
    def setTotalScore(self):
        ...
        try:
            ...
            runner.add_task(
                ...
                "score_dataset_and_code_availability",
                self.url,
                self.datasetURL,
                self.githubURL,
            )
            runner.add_task(self.reviewedness, "evaluate", self.url, self.githubURL)
            ...
            for metric, result, error in results:
                ...
                if error:
                    ...
                    if not actual_metric:
                        logger.warning(
                            f"Could not find metric object for {metric_name}"
                        )
                        continue
                    ...
                    if isinstance(result, tuple) and len(result) == 2:
                        ...
                        logger.debug(
                            f"{metric_name}: score={score}, latency={latency}ms"
                        )
                    elif result is None:
                        ...
                        actual_metric.metricLatency = latency
                        logger.debug(
                            f"{metric_name}: score={score}, latency={latency}ms (from returned metric)"
                        )
                        ...
                        actual_metric.metricLatency = latency
                        logger.debug(
                            f"{metric_name}: score={score}, latency={latency}ms (dict result)"
                        )
                    else:
                        logger.warning(
                            f"{metric_name} returned unexpected result type: {type(result)}"
                        )
                        actual_metric.metricScore = 0.0
                        actual_metric.metricLatency = 0
    ...
    def printScores(self):
        output = {
            ...
            "dataset_and_code_score": round(
                self.availableDatasetAndCode.getMetricScore(), 3
            ),

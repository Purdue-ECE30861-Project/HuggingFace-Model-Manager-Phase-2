...
async def index(request: Request, offset: int = Query(0)):
    ...
    try:
        ...
        artifacts_data = await fetch_through_middleware(
            "/artifacts",
            method="POST",
            data={"name": "*"},
        ...
        for artifact in artifacts_data:
            if artifact.get("metadata", {}).get("type") == "model":
                ...
                rating = await fetch_through_middleware(f"/artifact/model/{artifact_id}/rate")
...
@webpage_router.get("/artifact/{artifact_type}/{artifact_id}", response_class=HTMLResponse)
async def artifact_detail(request: Request, artifact_type: str, artifact_id: str):
    ...
    try:
        ...
        artifact = await fetch_through_middleware(f"/artifacts/{artifact_type}/{artifact_id}")
        if not artifact:
        ...
        lineage = None
        if artifact_type == "model":
            lineage = await fetch_through_middleware(f"/artifact/model/{artifact_id}/lineage")
        ...
        return templates.TemplateResponse(request=request, name="artifact_detail.html", context=context)
...
@webpage_router.post("/artifact/{artifact_type}/{artifact_id}/update", response_class=HTMLResponse)
    ...
    try:
        ...
        if result:
            ...
            return templates.TemplateResponse(
                ...
                context={
                    "request": request,
                    "artifact": {"metadata": update_data["metadata"], "data": update_data["data"]},
                    "artifact_type": artifact_type,
                    "artifact_id": artifact_id,
...
@webpage_router.delete("/artifact/{artifact_type}/{artifact_id}", response_class=HTMLResponse)
...
@webpage_router.post("/artifact/{artifact_type}/{artifact_id}/license-check", response_class=HTMLResponse)
